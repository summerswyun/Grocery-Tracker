      ...
      <div className="bg-white w-full max-w-md rounded-t-3xl p-6 pb-8 space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div><label className="text-xs text-stone-400 font-semibold">구매자</label><select value={form.buyer} onChange={(e) => set("buyer", e.target.value)} className="w-full mt-1 border border-stone-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-400">{MEMBERS.map((m) => <option key={m}>{m}</option>)}</select></div>
          </div>
